#' O3_flux_h - Calculates hourly deposition of O3 on vegetation
...
#' @return Hourly data of O3 deposited (g m-2 yr-1)
#' @export
#'
#' @examples
#'
#' data(Bizkaia_data)
#' O3_flux_h(x = Bizkaia_data)
#'
O3_flux_h <- function(x){
  db <- x
  ...
  Rb_O3 <- 2*(Sc_O3^(2/3))*(Pr^(-2/3))*((k*u_.)^(-1))
  ...
  r_m_O3[db$LAI_Total <= median(db$BAI)] <- Resistance_out_of_leaf # s/m
  ...
  r_t_O3[db$LAI_Total <= median(db$BAI)] <- Resistance_out_of_leaf # s/m
  # Canopy resistance
  # 1/Rc = 1/(r_s+r_m)+1/r_soil+1/r_t
  Rc_O3 <- 1/(1/(r_s+r_m_O3)+1/r_soil+1/r_t_O3)
  ### Total Resistance (Rt)
  Rt_O3 <- Ra+Rb_O3+Rc_O3
  ##################### Deposition Velocity (Vd)
  Vd_O3 <- 1 /Rt_O3
  ##################### F (Flux hourly)
  Flux_O3_hourly <- db$O3/1000000*Vd_O3*t_s*No_rain_event*db$LAI_Total# g/m2/h = g/m3 * m/s * s/h * unitless * unitless
  myresults <- data.frame(Dates=db$Dates, Flux_O3_hourly=Flux_O3_hourly)
  return(myresults)
}
