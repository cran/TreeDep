#' NO2_flux_h - Calculates hourly deposition of NO2 on vegetation
...
#' @return Hourly data of NO2 deposited (g m-2 yr-1)
#' @export
#'
#' @examples
#'
#' data(Bizkaia_data)
#' NO2_flux_h(x = Bizkaia_data)
#'
NO2_flux_h <- function(x){
  db <- x
  ...
  Rb_NO2 <- 2*(Sc_NO2^(2/3))*(Pr^(-2/3))*((k*u_.)^(-1))
  Rb_CO2 <- 2*(Sc_CO2^(2/3))*(Pr^(-2/3))*((k*u_.)^(-1))
  ### Canopy Resistance (Rc)
  # Stomata Resistance (r_s)
  Resistance_out_of_leaf <- 2800
  #Divide solar radiation above the canopy into visible (PAR) and near-infrared portions
  #Divide PAR above the canopy into direct beam and diffuse radiation
  theta <- 60 # solar zenith angle
  #E <- Extinction coefficient (= 0.185)
  E <- 0.185
  #m = Optical air mass
  ...
  rh <- db$Hum / 100
  # b1 =Zero intercept when A is equal to or less than zero
  ...
  ### ko = Michaelis-Menten coefficients for O2(= 295 millibars at 25 C)
  ...
  r_m_NO2[db$LAI_Total <= median(db$BAI)] <- Resistance_out_of_leaf # s/m
  ...
  r_t_NO2[db$LAI_Total <= median(db$BAI) ] <- Resistance_out_of_leaf # s/m
  # Canopy resistance
  # 1/Rc = 1/(r_s+r_m)+1/r_soil+1/r_t
  Rc_NO2 <- 1/(1/(r_s+r_m_NO2)+1/r_soil+1/r_t_NO2)
    ### Total Resistance (Rt)
  Rt_NO2 <- Ra+Rb_NO2+Rc_NO2
  ##################### Deposition Velocity (Vd)
  Vd_NO2 <- 1 /Rt_NO2
    ##################### F (Flux hourly)
  Flux_NO2_hourly <- db$NO2/1000000*Vd_NO2*t_s*No_rain_event*db$LAI_Total # g/m2/h = g/m3 * m/s * s/h * unitless * unitless
  myresults <- data.frame(Dates=db$Dates, Flux_NO2_hourly=Flux_NO2_hourly)
  return(myresults)
}
