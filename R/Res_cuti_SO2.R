...
#' @return Hourly data of cuticular resistance for SO2 (s m-1)
#' @export
#'
#' @examples
#'
...
Res_cuti_SO2 <- function(x){
  db <- x
  Resistance_out_of_leaf <- NA # 2800
  LAI_Total <-  db$BAI + db$LAI
  r_t_SO2 <- rep(8000 , nrow(db))
  r_t_SO2[LAI_Total <= median(db$BAI) ] <- Resistance_out_of_leaf # s/m
  r_cuti_SO2 <- cbind.data.frame(Dates = db$Dates, Resist_cuti = r_t_SO2)
  return(r_cuti_SO2)
}
