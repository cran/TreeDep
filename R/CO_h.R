#' CO_flux_h - Calculates hourly deposition of CO on vegetation
...
#' @return Hourly data of CO deposited (g m-2 yr-1)
#' @export
#'
#' @examples
#'
#' data(Bizkaia_data)
#' CO_flux_h(x = Bizkaia_data)
#'
CO_flux_h <- function(x){
  db <- x
  ...
  myresults <- data.frame(Dates=db$Dates, Flux_CO_hourly=Flux_CO_hourly)
  return(myresults)
}
